...
try {
  const {data} = await axios.get("http://localhost:5505/api/blogs/my-blogs",{
  ...
  const handleDelete=async(id)=>{
    await axios.delete(`http://localhost:5505/api/blogs/delete/${id}`,{
      withCredentials: true,
    }).then((res)=>{
      toast.success(res?.data?.message || "Blog deleted successfully");
      setMyBlogs((value)=>value.filter((blog)=>blog._id!==id));
    }).catch((error)=>{
     console.log(error?.data?.message || "An error occured while deleting.")
    })
  }
  return (
    <div>
      <div className="container mx-auto my-12 p-4 flex justify-center items-center w-[70%]">
